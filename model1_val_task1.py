...
def task1(args):
    ...
    task1_dataset_val = HuMobDatasetTask1Val('./data/enriched_human_mobility_100k.parquet')
    ...
    checkpoint = torch.load(args.pth_file, map_location=device)
    # Check if it's a dictionary containing the state_dict, or just the weights directly
    if isinstance(checkpoint, dict) and 'model_state_dict' in checkpoint:
        model.load_state_dict(checkpoint['model_state_dict'])
    elif isinstance(checkpoint, dict) and 'state_dict' in checkpoint:
        model.load_state_dict(checkpoint['state_dict'])
    else:
        # Fallback if it actually is just the weights
    ...
    with torch.no_grad():
        for data in tqdm(task1_dataloader_val):
            ...
            # --- NEW FEATURES ---
            data['lda'] = data['lda'].to(device)
            data['density'] = data['density'].to(device)
            data['weekend'] = data['weekend'].to(device)
            data['motif'] = data['motif'].to(device)
            
            output = model(data['d'], data['t'], data['input_x'], data['input_y'], data['time_delta'], data['lda'], data['density'], data['weekend'], data['motif'],data['len'])
