...
class HuMobDatasetTask1Train(Dataset):
    def __init__(self, path):
        ...
        for uid, traj in tqdm(self.df.groupby('uid')):
            ...
            # --- NEW: Mask Spatial Features (CRITICAL TO PREVENT LEAKAGE) ---
            lda_feat[need_mask_idx] = 0.0
            density[need_mask_idx] = 0.0
            ...
            # --- NEW: Append new arrays ---
            self.lda_array.append(lda_feat)
            self.density_array.append(density)
            self.weekend_array.append(weekend)
            self.motif_array.append(motif)
    ...
    def __getitem__(self, index):
        ...
        return {
            ...
            'label_y': label_y,
            'len': len,
            'lda': torch.tensor(self.lda_array[index]),
            'density': torch.tensor(self.density_array[index]),
            'weekend': torch.tensor(self.weekend_array[index]),
            'motif': torch.tensor(self.motif_array[index])
        }
